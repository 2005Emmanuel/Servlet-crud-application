...
//        processRequest(request, response);
        ...
        crud_DB_utils utils = new crud_DB_utils();
        utils.set_title(title);
        utils.set_description(description);

        int status = new DB_connection().insert(utils);
        if (status > 0) {
            ...
            out.println("an error occured nothing was created");
        }
    }
